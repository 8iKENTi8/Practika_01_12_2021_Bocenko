...
{
    public partial class Yclyga : UserControl
    {
        public Yclyga()
        {
            InitializeComponent();
        ...
        private void Yclyga_Load(object sender, EventArgs e)
        {
            ReloadDB();
        }
                ...
                if (e.ColumnIndex == 3)
                {
                    ...
                    if (task == "Update")
                    {
                            ...
                            MessageBoxIcon.Question) == DialogResult.Yes)
                        {
                            ...
                            DB db = new DB();
                            MySqlCommand command = new MySqlCommand("UPDATE `услуга` SET `id_y` = @ul," +
                                " `Название` = @ul1," +
                                " `Стоимость` = @ul2 WHERE `услуга`.`id_y` = @id", db.getConnection());
                ...
                if (e.ColumnIndex == 4)
                {
                    ...
                    if (task == "Delete")
                    {
                            ...
                            MessageBoxIcon.Question) == DialogResult.Yes)
                        {
                            ...
                            DB db = new DB();
                            MySqlCommand command = new MySqlCommand("DELETE FROM `услуга`" +
                                " WHERE `услуга`.`id_y` = @ul ", db.getConnection());
